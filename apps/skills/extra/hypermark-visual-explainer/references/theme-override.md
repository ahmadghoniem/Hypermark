...
```html
<meta name="hypermark-theme" content="host">
```
...
Hypermark handles dark/light via theme classes, not `prefers-color-scheme`. The standalone defaults above are the light theme. When embedded in raw HTML annotation mode (with the `hypermark-theme` meta opt-in), the active theme's tokens override automatically — no media query needed in the generated HTML.
