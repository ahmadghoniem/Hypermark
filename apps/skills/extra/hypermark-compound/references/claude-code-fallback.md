...
exists in `~/.claude/projects/` rather than the Hypermark plans directory (`$HYPERMARK_DATA_DIR/plans/` or `~/.plannotator/plans/`).
...
the Hypermark plans directory (`${HYPERMARK_DATA_DIR:-~/.plannotator}/plans/`) for existing `compound-planning-report*.html` files. If
...
to the Hypermark plans directory (`${HYPERMARK_DATA_DIR:-~/.plannotator}/plans/`, create the directory if it doesn't exist) so that
