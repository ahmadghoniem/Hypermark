...
description: >
  Analyze a user's Hypermark plan archive to extract denial patterns, feedback
  ...
  reasons when Hypermark data is unavailable.
---

# Compound Planning Analysis

You are conducting a comprehensive research analysis of a user's Hypermark plan
...
1. **Hypermark mode (first-class)** — Determine the Hypermark data directory:
   use `$HYPERMARK_DATA_DIR` if set, otherwise `~/.hypermark`. Check the
...
2. **Claude Code fallback mode** — If the Hypermark archive is absent or
...
3. **Neither available** — Ask the user for their Hypermark plans directory or
...
**Hypermark mode:** Verify the plans directory contains `*-denied.md` files. If
...
```
ls ${HYPERMARK_DATA_DIR:-~/.hypermark}/plans/compound-planning-report*.html
```
...
**Claude Code fallback mode:** The Hypermark inventory fields above do not apply.
...
instead of Hypermark markdown files.
...
- **Footer:** After section 7. Top border, centered italic Playfair Display tagline
  summarizing the corpus (e.g., "Analysis of X denied plans from the Hypermark
  archive.").
...
- **Claude Code fallback mode:** Explicitly label the report source as Claude Code
  `ExitPlanMode` denial reasons. Do not fabricate Hypermark-only fields such as
...
instead of Hypermark file counts.
...
and writes them to a file that Hypermark's `EnterPlanMode` hook can inject into
...
```
${HYPERMARK_DATA_DIR:-~/.hypermark}/hooks/compound/enterplanmode-improve-hook.txt
```
...
- **Data source priority:** Hypermark is the first-class path. Claude Code log
  analysis is the secondary path for users without Hypermark archives.
