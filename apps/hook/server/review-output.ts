 ...
 * the output directly, so it inherits this delivery. That is why this
 * currently returns true uniformly — the function exists as the seam where
 * an origin whose relay drops approve-time output would be keyed off, so
 * the advert can never outrun delivery for it
