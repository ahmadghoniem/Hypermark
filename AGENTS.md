...
│   │   ├── tailscale-serve.ts    # --tailscale orchestration
...
│   │   ├── webmcp/               # Browser-agent tool surface
│   │   ├── utils/                # parser.ts, storage.ts, planSave.ts, agentSwitch.ts, planDiffEngine.ts, planAgentInstructions.ts
...
│   │   ├── env-aliases.ts        # PLANNOTATOR_* -> HYPERMARK_* precedence rule (pure)
│   │   ├── env-aliases-apply.ts  # The only module allowed to mutate process.env; import it first from an entry point
...
Every `PLANNOTATOR_*` name still works as a deprecated alias: one shim
(`@hypermark/shared/env-aliases-apply`, imported first from the process entry
point) copies each legacy value onto its `HYPERMARK_*` name once at startup.
The current name wins whenever it is **set**, empty included, so
`HYPERMARK_DATA_DIR= hypermark review` cannot silently obey a stale
...
**Runtime coverage:** the feature ships on the Bun server + Claude Code CLI path. `hypermark annotate http://localhost:5173` probes live-first through the shared `live-probe` module and recognizes `--app`/`--static` via `parseAnnotateArgs`'s `liveFlags` opt-in.
