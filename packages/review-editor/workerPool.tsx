...
const highlighterOptions: WorkerInitializationRenderOptions = {
  ...
  // Wrap tokens with `data-char` so token-level interactions work (token
  // hover). Highlighting runs in the worker, and the
  ...
  // render highlighted but un-interactable.
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (!isReadyRef.current) {
        console.warn('Hypermark: highlight worker pool not ready after 5s — rendering without waiting.');
...
export function useWorkerPoolThemeSync(theme: { dark: string; light: string } | undefined): void {
  ...
  useEffect(() => {
    ...
    workerPool.setRenderOptions({ theme }).catch((err) => {
      ...
      console.warn('Hypermark: failed to sync highlight theme to worker pool', err);
    });
