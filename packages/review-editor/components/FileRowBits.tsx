...
export const CommittedDot: React.FC = () => (
  <span className="size-4 flex items-center justify-center shrink-0" title="Committed" aria-label="Committed">
...
export const TruncatedPath: React.FC<{ path: string }> = ({ path }) => {
  ...
  return (
    <span className="flex items-center min-w-0">
      ...
      <span className="shrink-0 max-w-full truncate">{path.slice(slash)}</span>
    </span>
  );
};
