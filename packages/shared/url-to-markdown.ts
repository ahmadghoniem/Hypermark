...
async function fetchRawText(url: string): Promise<string | null> {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 (compatible; Hypermark/1.0; +https://plannotator.ai)" };
  try {
    let currentUrl = url;
...
async function fetchViaContentNegotiation(url: string): Promise<string | null> {
  ...
  const headers = {
    "User-Agent": "Mozilla/5.0 (compatible; Hypermark/1.0; +https://plannotator.ai)",
...
async function fetchViaTurndown(url: string): Promise<string> {
  ...
  const headers = {
    "User-Agent":
      "Mozilla/5.0 (compatible; Hypermark/1.0; +https://plannotator.ai)",
