...
- `components/html-viewer/bridge-script.lite.ts`: the same `ANNOTATION_HIGHLIGHT_CSS` and `BRIDGE_PROTOCOL_VERSION` with `BRIDGE_SCRIPT = ""`. Export subpath `@hypermark/ui/components/html-viewer/bridge-script.lite`. An alias target only (below).
