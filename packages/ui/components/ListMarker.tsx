...
export const ListMarker: React.FC<ListMarkerProps> = ({
  ...
  return (
    ...
    >
      ...
      {isCheckbox ? (
        checked ? (
          <svg className="size-4 text-success mt-0.75" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        ) : (
          <svg className={`size-4 text-muted-foreground/50 mt-0.75${interactive ? ' hover:text-muted-foreground transition-colors' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
