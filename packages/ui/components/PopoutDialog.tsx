  ...
  return (
    <Dialog.Root open={open} onOpenChange={handleOpenChange} modal={false}>
      <Dialog.Portal container={container ?? undefined}>
        ...
        <div
          className="fixed inset-0 z-chrome bg-black/50 backdrop-blur-[2px]"
          onClick={handleBackdropClick}
          aria-hidden="true"
        />
        <Dialog.Popup
          className={`fixed left-1/2 top-1/2 z-chrome -translate-1/2  bg-background border border-border rounded-xl shadow-2xl flex flex-col overflow-hidden ${className ?? 'w-[calc(100vw-4rem)] max-w-[min(calc(100vw-4rem),1500px)] max-h-[calc(100vh-4rem)]'}`}
          data-popout="true"
          initialFocus={false}
          {...dataAttributes}
        >
