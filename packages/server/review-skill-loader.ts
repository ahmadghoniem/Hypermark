 ...
 * Skill Loader
 *
 * Discovers Agent Skills in the user's *global* skill roots and serves the
 * comment-composer's skill-reference catalog (`/api/skills`) plus the
 * SKILL.md body a human-only skill reference injects into exported feedback
 * (`/api/skills/content`).
 *
...
export interface ReferenceSkill {
   ...
   * later fails (skill deleted mid-session, unreadable file).
   */
  dir: string;
}
...
/**
 * Injection bound for a referenced skill's SKILL.md body: a giant SKILL.md
 * would blow up the exported feedback, so the body is TRUNCATED and the
 * truncation is reported, pointing the agent at the file itself.
 */
